...
var PuppyProvider = function (_a) {
    ...
    react_1.useEffect(function () {
        axios_1['default']
            .get('http://localhost:8000/api/puppies/')
    ...
    var updatePuppy = function (puppy) {
        var newPuppy = {
            id: puppy.id,
            name: puppy.name,
            breed: puppy.breed,
        ...
        var newPuppies = __spreadArrays(puppies)
        var index = newPuppies.findIndex(function (puppy) {
            return puppy.id === newPuppy.id
        })
        if (index > 0) {
            newPuppies[index].name = newPuppy.name
            newPuppies[index].breed = newPuppy.breed
            setPuppies(__spreadArrays(newPuppies))
        }
    }
    var deletePuppy = function (id) {
        ...
        console.log(puppies)
        var newPuppies = puppies.filter(function (puppy) {
            return puppy.id !== id
        })
